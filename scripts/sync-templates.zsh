...
# To push revised templates, merge/close that PR (or delete the sync branch), then re-run.
#
...
for repo in "${TARGET_REPOS[@]}"; do
  ...
  mkdir -p "$clone_dir/.github/ISSUE_TEMPLATE"
  for form in "$ISSUE_TEMPLATE_DIR"/*(.N); do
    add_if_absent "$form" "$clone_dir/.github/ISSUE_TEMPLATE/${form:t}" ".github/ISSUE_TEMPLATE/${form:t}"
  done

  # PR template and CONTRIBUTING — added only if the repo lacks its own.
  ...
  # The issue forms declare labels (epic/user-story/ticket); GitHub only applies labels
  # that already exist in the repo. Create any that are missing (non-destructive).
  for lbl in epic user-story ticket; do
    gh label create "$lbl" --repo "$slug" --color ededed \
      --description "AI Governance issue type" >/dev/null 2>&1 || true
  done
